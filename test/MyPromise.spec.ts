...
describe("promise.then(onFulfilled, onRejected)", function () {
  ...
  describe("2.2.2: If onFulfilled is a function (force async):", () => {
    it("2.2.2.1: must be called after promise is fulfilled, with promise’s value as its first argument.", () => {
      const onFulfilled = jest.fn();
  ...
  describe("2.2.3: If onRejected is a function (force async):", () => {
  ...
  describe("2.2.7: then must return a promise, continued", () => {
...
function promiseResolutionProcedure() {
  ...
  describe("2.3.2: If x promise, adopt its state.", () => {
    ...
    it.skip("If/when x is rejected, reject promise with the same value.", () => {
      expect.assertions(1);
      const promise2 = dfd.promise.then();
      let toReject;
      dfd.resolve(
